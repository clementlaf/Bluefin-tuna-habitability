...
class GroupModel(ABCmodel):
    def __init__(self, name):
        ...
        self.PATCH_SIZE = 512
        self.in_shape = (self.PATCH_SIZE, self.PATCH_SIZE, 13) # 3 physiques + NPP + 1 day length + 5 static
    ...
    def finetune(self, load_model):
        ...
        log("Compilation du modèle...")
        model.compile(optimizer=tf.keras.optimizers.AdamW(learning_rate=1e-5), loss=self.custom_weighted_spectral_loss, metrics=[self.NSE, self.filtered_loss], jit_compile=True)
...
if __name__ == "__main__":
    model = GroupModel(name="hmlmeso_medit")
    model.train()
    # model.finetune(load_model="/scratch/fra1831/modeles/Unet_v6.4/best.keras")
